...
contract crowdfunding{
    ...
    function finishFunding() public {
        ...
        //refund those users who didn't make threshold
            ...
            price = (goal /int(trueLength));
        }
        //uint[] memory listOfRefunds = new uint[](trueLength); //moving values to memory
        int distance;
        if(price > finalValue/2){
            distance = finalValue - price;    
        }else{
            distance = price;
        }
        uint iter = 0;
        uint indexpos = 0;
        uint usersLength = trueLength;
        int semirandom = int(usersLength - 1 - int256(keccak256(block.timestamp, block.difficulty))%((21000*usersLength)/(finalValue-indexpos)));
        while (trueLength > 0){
            if(allFunders[iter] != address(0)){
                if(indexpos > usersLength - 1-(21000*usersLength)/(finalValue-indexpos))
                    allFunders[iter].transfer(addressToValue[allFunders[iter]].fund - (uint(price + int(((2*int(indexpos)*price)/int(usersLength) - price)*distance)/price)+1));
                }else{
